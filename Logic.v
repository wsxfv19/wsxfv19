...
Proof.
  intros [|n].
-left. reflexivity.
-right. induction m. +reflexivity. +inversion H.
Qed.
(** [] *)
...
(* FILL IN HERE *)
(** [] *)
Lemma A:forall P Q K:Prop  ,
(P->Q)->(Q->K)->(P->K).
Proof.
intros P Q K H H1 H2.
...
(** Some of Coq' tactics treat [iff] statements specially, avoiding
...
+intros H. inversion H.
+simpl.  intros [H1 | H2].
{exists x'. rewrite H1. split. reflexivity.  left. reflexivity.
} apply IHl' in H2.  inversion H2. exists x.
destruct H. split. apply H. right. apply H0.
-induction l as [|x' l' IHl' ].
+intros [H1 H2]. inversion H2. inversion H0.
...
-intros H. induction l.
+simpl. destruct H. {inversion H. }apply H.
+simpl. simpl in H. destruct H.
...
intros T P l .
split.
-intros H. induction l as [|x].
+simpl. simpl in H.  reflexivity. 
+simpl. split.
{ apply H. simpl. left. reflexivity. 
}apply IHl. simpl in H. intros  x1. intros H1. apply H. right. apply H1.
-intros H. induction l as [|x].
+simpl. intros x H1. inversion H1.
+simpl. simpl in H. destruct H. intros x0. intros H1.  
destruct H1.
{rewrite<-H1. apply H.
...
Lemma I:forall {X:Type} (a:X)(l:list X)(test:X->bool),
(test a && forallb test l)%bool = true<->test a=true /\ forallb test l =true.
Proof.
Admitted.
...
-intros H. induction l.
+simpl. reflexivity.
+simpl. split. {simpl in H.  apply I in H. destruct H. apply H.
}apply IHl. simpl in H. apply I in H. destruct H. apply H0. 
-intros H. induction l.
+simpl. reflexivity.
+simpl. unfold andb. apply I.
split.
{simpl in H. destruct H. apply H.
}simpl in H. destruct H. apply IHl. apply H0.
Qed.
...
Proof.
  ...
  +destruct m.
inversion H.
inversion H.  eapply IHn in H.  rewrite H. reflexivity.
...
+destruct m.
{intros  H . inversion H.
}intros H. inversion H. simpl. apply IHn in H1. inversion H. rewrite H2 in H1.
apply H1.
Qed.
...
(** $Date: 2017-09-19 11:14:06 -0400 (Tue, 19 Sep 2017) $ *)
...
Fixpoint rev_append  (l1 l2 : natlist ) : natlist :=
  match l1 with
  | [] => l2
  | x :: l1' => rev_append l1' (x :: l2)
  end.

Definition tr_rev (l : natlist) : natlist :=
  rev_append l [].
...
induction l.
-simpl. reflexivity.
-simpl. rewrite<-IHl. unfold tr_rev. simpl. Abort. 
...
Lemma T: forall b1,false = b1&&false.
Proof.
intros b1.
induction b1.
-simpl. reflexivity.
...
Proof.
  intros b1 b2.
split.
-intros H1. split. +rewrite<-H1. induction b1 . {rewrite->H1. reflexivity.
}inversion H1. +rewrite<-H1. induction b2. {rewrite->H1. reflexivity.
}apply T. 
-intros [H1 H2].
...
Proof.
  intros b1 b2.
split.
-intros H1. unfold orb in H1. induction b1.
+left. apply H1.
+right. apply H1.
-intros [H1|H2].
+unfold orb . induction b1. 
{apply H1. }inversion H1.
+unfold orb . induction b2.
{induction b1. reflexivity. reflexivity.
}induction b1. reflexivity. inversion H2.
Qed.
...
-simpl. apply IHx.
Qed.
Lemma AT :forall x y:nat, 
 x <> y <->S x <>S y.
Proof.
intros x y.
unfold not.
split. intros H.
-intros H1. inversion H1. apply H. apply H2.
-intros H. intros H1. rewrite H1 in H. apply H. reflexivity.
Qed.
...
+induction y.
{unfold not. intros H1. inversion H1.
}unfold not. intros H1. rewrite->H1 in H.  simpl in H. unfold not in IHx. apply IHx.
rewrite<-H1. rewrite->AS in H. inversion H. rewrite->AS in H. inversion H.
...
+destruct y.
{simpl. reflexivity.
}simpl. eapply IHx. apply AT. apply H. 
Qed.
...
Fixpoint beq_list (l1 l2 : natlist ) : bool:=
match l1 with
|nil=>match l2 with
      |nil =>true 
      |b::l2'=>false
...
|a::l1'=>match l2 with
         |nil=>false
         |b::l2'=> match (beq_nat a b) with
                    |true => beq_list l1' l2'
                    |false => false
end
end
end.
Lemma Q :forall (l1 l2:natlist) (n n0:nat), 
n=n0 /\ l1=l2 -> (n::l1) =(n0::l2) .
Proof.
intros l1 l2 n n0.
intros H. inversion H. rewrite H0. rewrite H1. reflexivity.
Qed.
Lemma O :forall (l1 l2:natlist) (n n0:nat), 
beq_list (n :: l1) (n0 :: l2) = true ->beq_nat n n0=true/\ beq_list l1 l2=true.
Proof.
intros l1 l2 n n0.
split.
...
+destruct n0.
{intros H. inversion H.
}intros H. eapply IHn in H. 
simpl. apply H.
-Admitted.

Lemma wq :forall n n0:nat,beq_nat n n0 = true->n=n0.
Proof.
  -induction n;intros.
  +destruct n0. 
  {reflexivity.
...
Lemma beq_list_true_iff :
    forall l1 l2, beq_list l1 l2 = true -> l1 = l2.
Proof. 
induction l1. 
-destruct l2. 
{reflexivity.
...
+intros H. inversion H.
+intros H.
apply O in H. destruct H. apply wq in H. rewrite H. apply Q.
split. {reflexivity. 
}apply IHl1. apply H0.
